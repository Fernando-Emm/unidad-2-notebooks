...
Vx= r*cos(θ)
Vy=r*sin(θ)
z(x) = ((Vx)*x, h0 +(Vy)*x-((9.81*x^2)/2))
zy(x) = (h0+(Vy)*x -((9.81*x^2)/2))
zx(x)=(Vx*x)
	anim3 = @animate for t in 0:0.1:10
		plot(z.(range(-(2),0,step= 0.1).+t),
			legend= false, title= L"Tiro_parabolico", lw=3, ls=:dot)

scatter!([zx(t)], [zy(t)])
		x_max=(Vx*((2*Vy/9.81+ ((2*Vy/9.81)^2+ 10 * h0/9.81)^(1/2))/2))
		y_max=(h0+ (Vy)*(Vy/9.81)-9.81*(Vy/9.81)^2/2)
		xlabel!("Dist")
		ylabel!("high")
		
	end
	gif(anim3, "Tiro_parabolico.gif", fps=10)
...
function parabola(h1,r,θ,t)																#le damos los parámetros h1, r theta y t 
	
	Vrx = r * cos(θ)                                                                    #velocidad en el eje x.
	Vry = r * sin(θ) 																	#velocidad en el eje y.	

p(x) = ((Vrx) * x, h1 + (Vry) * x - ((9.81 * x^2) / 2))                                 #vector posición.
py(x) = (a + (Vry) * x - ((9.81 * x^2) / 2))                                            #componente en y del vector posición.
px(x) = (Vrx * x)                                                                       #componente en x del vector posición

	anim4 = @animate for t in 0:0.1:5                                                    #Para cada t en el intervalo de 0 a 5 con "pasos" de 0.1.
		plot(p.(range(-(3),0,step = 0.1).+t),                                           
		legend = false, title = "Función de tiro parabólico", lw = 3, ls=:dot)                    

scatter!([px(t)],[py(t)])                                                               #Graficamos el tiro parabolico.

	x_max = (Vrx * ((2 * Vry/9.81 + ((2 * Vry/9.81)^2 + 8 * a/9.81)^(1/2))/2))          #valor máximo en el eje x 
	y_max = (h1 + (Vry) * (Vry/9.81) - 9.81 * (Vry/9.81)^2/2)                           #valor máximo en el eje y
		
		xlabel!("Dist")                                                                 #el eje x será la distancia
		ylabel!("hihgt")                                                                #el eje y será la altura
		xlims!(0,x_max + 1)                                                           #le damos  límites del eje x.
		ylims!(0,y_max + 1)                                                           #le damos límites del eje y.
		
end

	gif(anim4, "Tiro_Parabólico_2.gif", fps = 10)                                      #Creamos la variable tiro, como un gif llamado "anim4".
...
default(legend = false, title = "h(x,y) = cos(x) + sin(y)")       #generamos una .
...
# ╔═╡ 2ac87a6c-d821-459e-8229-eb81890644e9
